...
class InstallerApp:
    ...
    def setup_ui(self):
        ...
        # Welcome text
        tk.Label(self.content_frame, text="Welcome to the PlanIFlow Installer.", font=("Segoe UI", 12)).pack(anchor="w", pady=(0, 20))
    ...
    def install_process(self, dest_path):
        try:
            ...
            if self.shortcut_var.get():
                ...
                # Find the main exe
                exe_path = None
                # Look for the main exe. It should be PlanIFlow_2.1.0.exe (as folder) -> PlanIFlow.exe? 
                # Or PlanIFlow_2.1.0.exe.exe? 
                # We will search for the largest .exe file in the root, or specific name if known.
                # Based on build.bat: --name="PlanIFlow_2.1.0.exe" in directory mode usually implies the exe is inside a folder of that name.
                ...
                # If 'PlanIFlow_2.1.0.exe.exe' exists, use it.
                main_exe = "PlanIFlow_2.1.0.exe.exe" # PyInstaller often appends .exe if name has extension?
                if main_exe not in candidates:
                     main_exe = "PlanIFlow_2.1.0.exe" # if no extra .exe added
